...
#define SINCOS_MAX_AMPLITUDE		1.65		// sqrt(sin^2 + cos^2) has to be smaller than this
...
#if (AS5047_USE_HW_SPI_PINS) || (ICMHM_USE_HW_MOSI_PIN)|| (MT6816_USE_HW_SPI_PINS) || (AD2S1205_USE_HW_SPI_PINS)
  	#ifdef HW_SPI_DEV
		...
		#define SPI_SW_MISO_GPIO			GPIOA
		#define SPI_SW_MISO_PIN				6
		#define SPI_SW_MOSI_GPIO			GPIOA
		#define SPI_SW_MOSI_PIN				7
		#define SPI_SW_SCK_GPIO				GPIOA
		#define SPI_SW_SCK_PIN				5
		#define SPI_SW_CS_GPIO				GPIOA
		#define SPI_SW_CS_PIN				4
	#endif
#else
...
typedef enum {
	...
	ENCODER_MODE_ICMHM_SPI,				// 方便VESC Tool界面上操作
	ENCODER_MODE_MT6816_SPI
} encoder_mode;
...
static const SPIConfig icmhm_spi_cfg = {
			NULL,
			SPI_SW_CS_GPIO,
			SPI_SW_CS_PIN,
...
void encoder_mhm_reset_multiturn(void) {
	mhm_reset_multiturn = true;
}
...
void encoder_init_icmhm_spi(void) {
	TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
...
#if (ICMHM_USE_HW_SPI_PINS)
	palSetPadMode(SPI_SW_MOSI_GPIO, SPI_SW_MOSI_PIN, PAL_MODE_ALTERNATE(6) | PAL_STM32_OSPEED_HIGHEST);
#endif

	//Start driver with ICMHM SPI settings
	spiStart(&HW_SPI_DEV, &icmhm_spi_cfg);
#else
	palSetPadMode(SPI_SW_MISO_GPIO, SPI_SW_MISO_PIN, PAL_MODE_INPUT);
	palSetPadMode(SPI_SW_SCK_GPIO, SPI_SW_SCK_PIN, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
	palSetPadMode(SPI_SW_CS_GPIO, SPI_SW_CS_PIN, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
...
void encoder_tim_isr(void) {	

	{
		...
		spi_begin();
#ifdef HW_SPI_DEV
		abs_spi_dma_rx_[0] = spiPolledExchange(&HW_SPI_DEV, abs_spi_dma_tx_[0]);
		abs_spi_dma_rx_[1] = spiPolledExchange(&HW_SPI_DEV, abs_spi_dma_tx_[1]);
		abs_spi_dma_rx_[2] = spiPolledExchange(&HW_SPI_DEV, abs_spi_dma_tx_[2]);
		abs_spi_dma_rx_[3] = spiPolledExchange(&HW_SPI_DEV, abs_spi_dma_tx_[3]);
		abs_spi_dma_rx_[4] = spiPolledExchange(&HW_SPI_DEV, abs_spi_dma_tx_[4]);
		abs_spi_dma_rx_[5] = spiPolledExchange(&HW_SPI_DEV, abs_spi_dma_tx_[5]);
#else
...
#ifdef HW_SPI_DEV
	if (mode == ENCODER_MODE_ICMHM_SPI) {
		static const uint8_t abs_spi_dma_tx_[6] = {0xA6, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    	uint8_t abs_spi_dma_rx_[6];
		...
		uint32_t mtval = ((abs_spi_dma_rx_[1] << 16) | (abs_spi_dma_rx_[2] << 8) | (abs_spi_dma_rx_[3] << 0));
		uint16_t angle = (abs_spi_dma_rx_[4] << 8) | (abs_spi_dma_rx_[5] << 0);
		angle >>= 4;
		pos = angle;
		spi_val = pos;

		last_enc_angle = 1.0f * mtval + ((float)pos * 360.0) / 4096.0;
		UTILS_LP_FAST(spi_error_rate, 0.0, 1./MT6816_SAMPLE_RATE_HZ);
		UTILS_LP_FAST(encoder_no_magnet_error_rate, 0.0, 1./MT6816_SAMPLE_RATE_HZ);
	}
...
#if AS504x_USE_SW_MOSI_PIN || AS5047_USE_HW_SPI_PINS || ICMHM_USE_HW_MOSI_PIN
...
#else
	{
		...
		for (i=0; i<datasize; i++) {
			...
			for (j=8; j>0; j--) {
				...
				palWritePad(SPI_SW_MOSI_GPIO, SPI_SW_MOSI_PIN, k?1:0);
				spi_delay();
				spi_delay();
				//rt_pin_write(PIN_M0_SCK, PIN_HIGH);
				palWritePad(SPI_SW_SCK_GPIO, SPI_SW_SCK_PIN, 1);
				spi_delay();
				spi_delay();
				...
				palWritePad(SPI_SW_SCK_GPIO, SPI_SW_SCK_PIN, 0);
				val_r |= k ? (1<<(j-1)) : 0x00;
			}
			data_rx[i] = val_r;
		}
